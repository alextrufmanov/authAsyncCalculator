...
1. Регистрация пользователя user01 с паролем psw_user_01 (если пользователь с именем user01 в БД уже существует, то будет получен ответ "{"error":"Can't create user"}" - попробуйте создать пользователя с другим именем, например psw_user_01)
```sh
